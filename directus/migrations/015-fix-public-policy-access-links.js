...
export async function up(knex) {
  await fixPublicPolicyAccessLinks(knex);
}

export async function down() {
  // Public product assets must remain available to the storefront.
}

async function fixPublicPolicyAccessLinks(knex) {
  if (
    !(await knex.schema.hasTable("directus_permissions")) ||
  ...
  const policyId = await ensurePublicAssetsPolicy(knex);
  await ensureAnonymousAccessWithPolicyId(knex, policyId);
...
async function ensureAnonymousAccessWithPolicyId(knex, policyId) {
  ...
  const row = await filterToExistingColumns(knex, "directus_access", {
    id: accessColumns.has("id") ? policyId : undefined,
    role: null,
    user: null,
    policy: policyId,
    sort: null,
  ...
  if (accessColumns.has("id")) {
    const existingById = await knex("directus_access").where({ id: policyId }).first();

    if (existingById) {
      await knex("directus_access").where({ id: policyId }).update(row);
      return;
    }
  }

  const existingByPolicy = await getAnonymousAccessByPolicy(knex, policyId, accessColumns);

  if (existingByPolicy && accessColumns.has("id")) {
    await knex("directus_access").where({ id: existingByPolicy.id }).update(row);
    return;
  }

  const insertRow = {
    ...row,
    id: row.id ?? (accessColumns.get("id")?.data_type === "uuid" ? randomUUID() : undefined),
  };

  await knex("directus_access").insert(
    await filterToExistingColumns(knex, "directus_access", insertRow),
  );
}

async function getAnonymousAccessByPolicy(knex, policyId, accessColumns) {
  const query = knex("directus_access").where({ policy: policyId });
